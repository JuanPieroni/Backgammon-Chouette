...
function GameBoard() {
  ...
  const handleNextRound = () => {
    ...
    if (totalScore !== 0) {
      ...
      alert(
        'La cuenta no da! deseas ajustarlo automaticamente?'
      );
    } else {
  ...
  const handleSaveRound = () => {
    ...
    if (totalScore !== 0) {
      alert(
        'La cuenta no da! Modificar puntaje?'
      );
      return;
    }
    ...
    <div className="game-board">
      ...
      <table>
        ...
        <tbody>
            ...
            <tr key={index}>
              ...
              <td>
                {editIndex !== index ? (
                  ...
                  <>
                    <button className="action-button" onClick={handleSaveRound}>
                      Guardar
                    </button>
                  </>
                )}
          ...
          <tr className="total-row">
            <td>Total</td>
            <td>{rounds.reduce((total, round) => total + round.A, 0)}</td>
            <td>{rounds.reduce((total, round) => total + round.B, 0)}</td>
            <td>{rounds.reduce((total, round) => total + round.C, 0)}</td>
            <td></td>
          </tr>
        </tbody>
      </table>
